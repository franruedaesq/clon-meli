...
export default function Paises() {
    ...
    const fetchPaises = async() => {
        let data = await fetch('https://cors-anywhere.herokuapp.com/https://api.mercadolibre.com/sites');
    ...
    let lista = paisData.map(item => <Pais meliID={item.id} name={item.name} key={item.id}></Pais>)
    return (
